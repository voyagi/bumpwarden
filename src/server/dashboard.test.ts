...
describe('the published policy', () => {
  ...
  /**
   * The card makes three promises about a body this page never renders, and the two are written in
   * different files by different hands. Nothing else holds them together, so a reworded disclosure
   * would leave the page promising in public what the issue no longer says.
   */
  it('promises about a brief body only what a real one carries', async () => {
    const page = await text(store, '/rubric');
    const score = scoreOf();
    // Read off the brief rather than off BRIEF_MODEL: the promise is that a body names the model
    // that wrote its own brief, and pinning it to the shipped constant would only mean this test
    // reddens the day the operator follows the ADR and moves the alias. The about page is where
    // the shipped constant is pinned.
    const brief = readyBrief();
    ...
    expect(page).toContain('names the model');
    expect(body).toContain(`Generated by the AI model ${brief.model}`);

    expect(page).toContain('read the brief before acting on it');
    expect(body).toContain('read it before acting on it');

    expect(page).toContain('a marker a machine can read');
    expect(body).toContain(generatedByMarker(brief.model));
  });
});
