...
function disclosure(model: string, confidence: string): string {
  return [
    `Generated by the AI model ${code(model)} through bumpwarden, from the release notes and the diff,`,
    "then checked against this repository's own call sites. No person has reviewed it: read it",
    `before acting on it. Confidence: ${confidence}.`,
  ].join(' ');
}
